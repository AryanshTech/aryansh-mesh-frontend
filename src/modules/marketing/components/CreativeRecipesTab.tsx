...
export function CreativeRecipesTab({ projectId, tenantId }: Props) {
  ...
  const onCreate = async () => {
    ...
    if (!draftRecipe.title.trim()) { toast.error(t('marketing.recipes.titleRequired')); return; }
    try {
  ...
  const onGenerate = async () => {
    ...
    if (!draftPackage.goal.trim()) { toast.error(t('marketing.recipes.goalRequired')); return; }
    try {
              ...
              {recipes.map((r) => (
                <tr
                  ...
                  className={cn(
                    ...
                    selected?.id === r.id && panelMode === 'view' ? 'bg-primary/5' : 'hover:bg-muted/30',
                  )}
                >
                  ...
                  <td className="px-4 py-2.5 text-muted-foreground">{t(`marketing.assets.types.${r.assetType}`)}</td>
                </tr>
              ))}
            </tbody>
          </table>
  ...
  return (
    <DetailDrawer
      ...
      footer={
        panelMode === 'create' ? (
          <div className="flex items-center justify-end gap-2">
            <Button variant="outline" onClick={closePanel}>{t('common.cancel')}</Button>
          ...
          <div className="flex justify-end">
            <Button variant="outline" onClick={closePanel}>{t('common.close')}</Button>
          </div>
        )
      }
    >
